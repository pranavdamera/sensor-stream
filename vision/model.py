...
class ChangeDetector(nn.Module):
    ...
    A 4-head spatial self-attention module sits at the bottleneck so the model
    can attend over spatially distributed change patterns before upsampling.
    H and W must be divisible by 8.
    """

    def __init__(self, num_attention_heads: int = 4) -> None:
        super().__init__()
        ...
        # Bottleneck + multi-head spatial attention
        self.bottleneck = _ConvBlock(128, 256)
        self.bottleneck_attn = BottleneckAttention(256, num_heads=num_attention_heads)
        ...
        # 1x1 head -- outputs single-channel logit map
    ...
    def forward(self, before: torch.Tensor, after: torch.Tensor) -> torch.Tensor:
        ...
        b = self.bottleneck_attn(self.bottleneck(self.pool(e3)))
